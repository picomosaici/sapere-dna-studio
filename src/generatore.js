...
(function (root) {
  ...
  function trainSAE(A, opts) {
    ...
    const normDec = () => { for (let m = 0; m < M; m++) { let nr = 0; for (let k = 0; k < d; k++) nr += Wd[k][m] * Wd[k][m]; nr = Math.sqrt(nr) || 1; for (let k = 0; k < d; k++) Wd[k][m] /= nr; } };
    normDec(); const opt = new Adam(lr);
    const encode = (z) => { const c = new Float64Array(M); for (let m = 0; m < M; m++) { let s = be[0][m]; for (let k = 0; k < d; k++) s += We[m][k] * z[k]; c[m] = relu(s); } return c; };
    for (let t = 0; t < steps; t++) {
    ...
    normDec();
    let active = 0, rmse = 0;
    for (const r of A) { const z = norm(r), c = encode(z); active += c.reduce((a, v) => a + (v > 1e-2 ? 1 : 0), 0); const rec = new Float64Array(d); for (let k = 0; k < d; k++) { let s = bd[0][k]; for (let m = 0; m < M; m++) s += Wd[k][m] * c[m]; rec[k] = s; } let e = 0; for (let k = 0; k < d; k++) e += (rec[k] - z[k]) ** 2; rmse += Math.sqrt(e / d); }
    const decode = (c) => { const rec = new Float64Array(d); for (let k = 0; k < d; k++) { let s = bd[0][k]; for (let m = 0; m < M; m++) s += Wd[k][m] * c[m]; rec[k] = s; } return rec; };
...
function deserialize(obj, vocab) {
  ...
  function buildSAEInterface(raw) {
    ...
    const encode = (z) => { const c = new Float64Array(M); for (let m = 0; m < M; m++) { let s = be[0][m]; for (let k = 0; k < d; k++) s += We[m][k] * z[k]; c[m] = relu(s); } return c; };
    const decode = (c) => { const rec = new Float64Array(d); for (let k = 0; k < d; k++) { let s = bd[0][k]; for (let m = 0; m < M; m++) s += Wd[k][m] * c[m]; rec[k] = s; } return rec; };
    return { M, d, We, Wd, be, bd, mean, std, norm, denorm, encode, decode, avgActive: raw.avgActive, rmse: raw.rmse };
  }

  const API = {
    makeVocab, encode, decode, LM, train, perplexity, generate,
    lastPosMatrix, trainSAE, characterizeConcepts, extractGenome, derive, intervene, seal,
